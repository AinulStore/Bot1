...
module.exports = {
            ...
            case 'remove':
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        let pp = 'https://telegra.ph/file/0a42649c18af14bb08615.jpg'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc ? String.fromCharCode(8206).repeat(4001) + groupMetadata.desc : '') :
                                (chat.sBye || this.bye || conn.bye || 'Bye, @user!')).replace('@user', await this.getName(user)).replace('@subject', await this.getName(id))
               
                            let wel = API('males', '/welcome2', {
                                profile: pp,
                                username: await this.getName(user),
                                background: 'https://telegra.ph/file/99721d2ed3f5436f5bcbe.jpg',
   ...
   conn.sendButtonImg(id, await(await fetch(action === 'add' ? wel : lea)).buffer(), text, wm, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Wellcome', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: linkig,
    mediaType: 2, 
    description: urlnya,
    title: "𝑆𝑢𝒉𝑢",
...
global.dfail = async (type, m, conn) => {
    let msg = {
        ...
        owner: `Perintah ini hanya dapat digunakan oleh _*Owner!1!1!*_`,
        ...
        rpg: `RPG tidak aktif, Silahkan hubungi Owner Untuk mengaktifkan fitur ini!`,
